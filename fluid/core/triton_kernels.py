...
# ---------------------------------------------------------------------------
# split_columns_to_chunks / merge_chunks_to_columns
#
# split:  [N, H] → [C*N, cs]  (contiguous;  cs = H // C)
# merge:  [C*N, cs] → [N, H]  (column-interleave)
#
# Each replaces C separate .copy_() kernel launches with a single launch.
# ---------------------------------------------------------------------------

@triton.jit
def _split_columns_kernel(
    src_ptr, dst_ptr,
    N, hidden_size, chunk_size,
    BLOCK_D: tl.constexpr,
):
    row = tl.program_id(0)
    chunk_id = tl.program_id(1)
    d_off = tl.arange(0, BLOCK_D)
    mask = d_off < chunk_size

    src_offset = row * hidden_size + chunk_id * chunk_size + d_off
    vals = tl.load(src_ptr + src_offset, mask=mask)

    dst_offset = (chunk_id * N + row) * chunk_size + d_off
    tl.store(dst_ptr + dst_offset, vals, mask=mask)


def split_columns_to_chunks(
    src: torch.Tensor,
    num_chunks: int,
    out: torch.Tensor,
) -> torch.Tensor:
    """Split [N, H] into [C*N, chunk_size] by column chunks (single kernel).

    Equivalent to::

        for c in range(C):
            out[c*N:(c+1)*N] = src[:, c*cs:(c+1)*cs]

    Args:
        src:        [N, H]  contiguous source
        num_chunks: C
        out:        [C*N, cs]  contiguous destination  (cs = H // C)
    """
    N, H = src.shape
    chunk_size = H // num_chunks
    assert H % num_chunks == 0 and out.shape == (num_chunks * N, chunk_size)

    if N == 0:
        return out

    BLOCK_D = min(triton.next_power_of_2(chunk_size), 2048)
    grid = (N, num_chunks)
    _split_columns_kernel[grid](
        src, out, N, H, chunk_size,
        BLOCK_D=BLOCK_D, num_warps=8,
    )
    return out


@triton.jit
def _merge_columns_kernel(
    src_ptr, dst_ptr,
    N, hidden_size, chunk_size,
    BLOCK_D: tl.constexpr,
):
    ...
    src_offset = (chunk_id * N + row) * chunk_size + d_off
    vals = tl.load(src_ptr + src_offset, mask=mask)

    dst_offset = row * hidden_size + chunk_id * chunk_size + d_off
    tl.store(dst_ptr + dst_offset, vals, mask=mask)


def merge_chunks_to_columns(
    src: torch.Tensor,
    num_chunks: int,
    N: int,
    out: torch.Tensor,
) -> torch.Tensor:
    """Merge [C*N, chunk_size] into [N, H] by column chunks (single kernel).

    Equivalent to::

        for c in range(C):
            out[:, c*cs:(c+1)*cs] = src[c*N:(c+1)*N]

    Args:
        src:        [C*N, cs]  contiguous source
        num_chunks: C
        N:          number of rows in output
        out:        [N, H]  destination  (H = C * cs)
    """
    chunk_size = src.shape[1]
    H = num_chunks * chunk_size
    assert out.shape == (N, H) and src.shape[0] == num_chunks * N
