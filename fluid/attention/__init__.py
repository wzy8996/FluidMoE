...
- backward.py: Backward operations with AllToAll + dX/dW scheduling
  - Output projection backward: chunked dX + sp2hp AllToAll + dW overlap
  - QKV projection backward: hp2sp AllToAll + chunked dX + dW overlap

Note: SDPA backward (grad_Q/K/V) is handled in fluid.layer.transformer via saved
computation graph, avoiding redundant SDPA forward in backward.

Key design principles:
- Forward uses P2P overlap for compute-communication overlap
- Backward uses AllToAll with chunked compute overlap
- Memory-efficient: save SDPA computation graph (logsumexp O(seq), not attention matrix O(seq²))
