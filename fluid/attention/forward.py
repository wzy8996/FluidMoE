...
try:
    _SDPA_HAS_ENABLE_GQA = 'enable_gqa' in inspect.signature(
...
def qkv_projection_p2p_forward(
    ...
    assert cp_plan is not None, "qkv_projection_p2p_forward requires CPPlan for cp_size>1"
    ...
    qkv_full = p2p_backend.alloc_recv_buffer(
        "attn_qkv_full", _qkv_numel, dtype, device
    ).view(seq_full, batch, groups_per_rank, group_size)

    _use_slots = cp_plan.qkv_send_slots is not None
    last_p2p_event = None

    # Per-region CCE hook (r1_f = QKV sp2hp). No-op when metrics disabled.
    from fluid.core.scheduler import get_backward_scheduler
    from fluid.core import overhead_profiler as _oh
    _sched = get_backward_scheduler()
    _r1f_start = _sched.begin_fwd_comm(comm_stream) if n_partners > 0 else None

    for i in range(n_partners):
        w_start = cp_plan.qkv_weight_offsets[i]
        ...
        # Overhead: P2P round Python orchestration. Zero overhead when disabled.
        _oh_tok = _oh.enter("overhead.fwd_tournament_round") if _oh.ENABLED else None
        try:
            ready_events[i].record(default_stream)
            ready_evt = ready_events[i]
            seq_start = cp_plan.qkv_recv_seq_starts[i]
    ...
    local_seq_start = cp_plan.my_rank * seq_local
    qkv_full[local_seq_start:local_seq_start + seq_local] = local_qkv

    # Close CCE event pair on comm stream, then bracket the default-stream wait.
    _sched.end_fwd_comm('r1_f', _r1f_start, comm_stream, end_evt=last_p2p_event)

    # GPU-level wait for all P2P to complete (no CPU blocking)
    _sched.record_fwd_wait('r1_f', last_p2p_event)

    if p2p_backend.needs_final_wait():
        p2p_backend.final_wait()
...
def scaled_dot_product_attention_forward(
    ...
    # PyTorch <2.5 SDPA has no enable_gqa kwarg; expand K/V heads manually.
    if enable_gqa and not _SDPA_HAS_ENABLE_GQA:
        ...
        output = F.scaled_dot_product_attention(
            query, key, value, attn_mask=None, dropout_p=0.0,
            is_causal=is_causal, scale=scale,
            **({'enable_gqa': enable_gqa} if _SDPA_HAS_ENABLE_GQA else {}),
        )
...
def output_projection_p2p_forward(
    ...
    assert cp_plan is not None, "output_projection_p2p_forward requires CPPlan for cp_size>1"
    ...
    p2p_backend = get_p2p_backend()
    _use_slots = cp_plan.proj_recv_slots is not None

    send_data_list = [
        attn_output[p * seq_local:p * seq_local + seq_local]
        for p in partners
    ]
    if _use_slots:
        recv_bufs = cp_plan.proj_recv_slots
        recv_bufs_flat = cp_plan.proj_recv_flat
    else:
        recv_bufs = []
        recv_bufs_flat = []
        for i in range(n_partners):
            recv_buf = p2p_backend.alloc_recv_buffer(
                f"attn_proj_recv_{i}",
                seq_local * batch_size * heads_local * head_dim,
                dtype,
                device,
    ...
    # Per-region CCE hook (r2_f = output proj hp2sp). Record one comm pair
    # spanning the full loop; wait pairs are registered per-partner below.
    from fluid.core.scheduler import get_backward_scheduler
    from fluid.core import overhead_profiler as _oh
    _sched = get_backward_scheduler()
    _r2f_start = _sched.begin_fwd_comm(comm_stream) if n_partners > 0 else None

    for i in range(n_partners):
        recv_buf_i = recv_bufs[i]

        # Overhead: per-round P2P orchestration for output projection.
        _oh_tok = _oh.enter("overhead.fwd_tournament_round") if _oh.ENABLED else None
        try:
            with torch.cuda.stream(comm_stream):
                if i == 0:
        ...
        if i == 0:
            ...
            _sched.record_fwd_wait('r2_f', p2p_events[i - 1])
            prev_flat = recv_bufs_flat[i - 1]
    ...
    if n_partners > 0:
        last = n_partners - 1
        # Close r2_f comm pair on the comm stream, then bracket final wait.
        _sched.end_fwd_comm('r2_f', _r2f_start, comm_stream, end_evt=p2p_events[last])
        _sched.record_fwd_wait('r2_f', p2p_events[last])
        last_flat = recv_bufs_flat[last]
